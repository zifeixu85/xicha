...
test("custom media adapter refreshes JWT per request and matches backend fields", async () => {
  ...
  assert.match(requests[2].url, /\/api\/media-task\?taskId=image-task-1&pollToken=image-token$/);
  assert.match(requests[3].url, /\/api\/video-task\?taskId=video-task-1&taskType=video&pollToken=video-token$/);
});
